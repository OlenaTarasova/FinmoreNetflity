...
namespace ApiPositiveTest.Test
{
    [TestFixture]
    public class PostsCrudIDTests
    {
        ...
        private static readonly string BaseUrl =
            ...
            ?? "https://dev.emeli.in.ua";
 
        private static readonly string Username =
            Environment.GetEnvironmentVariable("WP_USERNAME")
            ?? throw new ArgumentNullException("WP_USERNAME is not set");
 
        private static readonly string Password =
            Environment.GetEnvironmentVariable("WP_PASSWORD")
            ?? throw new ArgumentNullException("WP_PASSWORD is not set");
        ...
        public async Task Setup()
        {
            ...
            _api = await _playwright.APIRequest.NewContextAsync(new()
            {
                ...
                ExtraHTTPHeaders = new Dictionary<string, string>
                {
                    ["Authorization"] = AuthHelper.GetBasicAuthHeader(Username, Password),
        ...
        [Test, Order(1)]
        public async Task Create_Post_Should_Return_Generated_ID()
        {
            ...
            Assert.That(response.Status, Is.EqualTo(201), "Post was not created");
 
            using var json = JsonDocument.Parse(await response.TextAsync());
            var root = json.RootElement;
 
            _postId = root.GetProperty("id").GetInt32();
 
            Assert.Multiple(() =>
            {
                Assert.That(_postId, Is.GreaterThan(0), "Post ID should be generated by server");
                Assert.That(root.GetProperty("status").GetString(), Is.EqualTo("draft"));
                Assert.That(
                    root.GetProperty("title").GetProperty("rendered").GetString(),
                    Does.Contain("New Post for ID test")
                );
            });
        }
        ...
        public async Task TearDown()
        {
            if (_postId > 0)
            {
                await _api.DeleteAsync($"/wp-json/wp/v2/posts/{_postId}?force=true");
            }
